...
//Returns: None
__global__ void computeRow(float* src,float* dest,int height,int pWidth,int radius,int bpp){
    int i;
...
//            bpp: The bits per pixel in the src image
//Returns: None
__global__ void computeColumn(uint8_t* src,float* dest,int co,int pWidth,int height,int radius,int bpp){
...
int main(int argc,char** argv){
    ...
    computeColumn<<<blockCount, blockSize>>>(img2,mid,i,pWidth,height,radius,bpp);
    ...
    computeRow<<<(height+255)/256, blockSize>>>(mid,dest,height,pWidth,radius,bpp);
